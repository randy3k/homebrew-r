  ...
  def install
    ...
    mkdir "build" do
      system "cmake", "..",
        ...
        "-DBoost_INCLUDE_DIR=#{Formula["boost150"].opt_include}",
        "-DCMAKE_INSTALL_PREFIX=#{prefix}/rstudio",
        "-DCMAKE_EXE_LINKER_FLAGS=-L#{Formula["openssl"].opt_lib} -L#{Formula["boost150"].opt_lib}",
    ...
    chmod 0755, bin/"rserver"

    (bin/"rstudio-server").write <<-EOS.undent
      #!/bin/sh

      testConfig() {
       `#{opt_bin}/rserver --test-config`
      }

      verifyInstallation() {
       `#{opt_bin}/rserver --verify-installation=1 --server-daemonize=0`
      }
    ...
    chmod 0755, bin/"rstudio-server"

    (prefix/"etc/com.rstudio.launchd.rserver.plist").write <<-EOS.undent
  ...
  def caveats
    <<-EOS.undent
      ...
      - To remove the launching daemon,
        ...
        you may need to also remove the old plist file

          sudo rm /Library/LaunchDaemons/org.rstudio.launchd.rserver.plist
      ...
      - In default, only users with id >1000 are allowed to login. You could relax this
        requirement by editing the `ProgramArguments` section of the file
        `/Library/LaunchDaemons/com.rstudio.launchd.rserver.plist`
